...
Full findings with the raw rows they cite: `feature-requests/FR-990-cap-journey-census.md` → *Raw Output Read*.
...
- **No automated tests for the checker (`tools.py`).** FR-990 AC-7 is open. This is a research instrument whose output is advisory; the tests are required before its `retire` rows drive any removal.
- No full run over 242; no comparison with the fast model (`mercury-2`) that was agreed — the classification has to be stabilised first (FR-990 Proposed Solution 1–5: category definitions to the model, code cap for the catch-all, cross-cutting handling, consumer hit-kind check, one exclusion bug).
- Nothing is retired by this PR. Each candidate goes through its own FR.

## Process notes

Graph and prompt were written through the repo's authoring route (`scripts/author.sh`, brief in `feature-requests/authoring-briefs/`), two revisions. Python was hand-written. Diaries: `docs/diary/2026-09-05-reflection-fr-990-*.md`, `docs/diary/diary-2026-09-05-the-recap-nobody-outside-could-read.md`. The first version of this description was a pasted commit message; an outsider-reader spike (plan §12) produced 33 "I don't understand" items against it and this rewrite.
